...
export async function getAccessToken() {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.error_description || data.error || 'Spotify refresh failed');
...
async function spotifyFetch(path, options = {}) {
  ...
  if (!res.ok) {
    const message = data?.error?.message || data?.error_description || `Spotify API error ${res.status}`;
...
export async function searchTrack({ artist, title }) {
  ...
  const params = new URLSearchParams({ q: query, type: 'track', limit: '5', market: 'DE' });
...
export async function getPlaylistTracks(limit = 100) {
  ...
  const params = new URLSearchParams({ limit: String(limit), fields: 'items(track(id,name,uri,artists(name),album(images)))' });
  const data = await spotifyFetch(`/playlists/${playlistId}/tracks?${params.toString()}`);
