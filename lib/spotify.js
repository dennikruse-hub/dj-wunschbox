...
export async function getAccessToken() {
  ...
  if (!res.ok) {
    throw new Error(data.error_description || data.error || 'Spotify Token Fehler');
  }
...
export async function exchangeCodeForToken(code) {
  ...
  if (!res.ok) {
    throw new Error(data.error_description || data.error || 'Spotify Login Fehler');
  }
...
export async function getPlaylistTracks() {
  ...
  if (cache.playlist && cache.playlistUntil > now) {
  ...
  cache.playlistUntil = now + 8 * 1000;
